...
if(!fs.existsSync("/Applications/4Casters/server2/db")){
  console.log("creating database00");
  fs.mkdir("/Applications/4Casters/server2/db", 0766, function(err){
      ...
      if(err){
        ...
        db = new Datastore({ filename: '/Applications/4Casters/server2/db/store.json', autoload: true });
      }
  });   
}else{
  copyFile('/Applications/4Casters/server2/db/store.json','/Applications/4Casters/server2/db/store_temp.json',function(err){
    if (err == null || err == undefined || err == '') {
      db = new Datastore({ filename: '/Applications/4Casters/server2/db/store.json', autoload: true });
      setTimeout(function () {
        fs.unlink('/Applications/4Casters/server2/db/store.json', function(err){
          console.log("err: "+err);
          if (!err){
            fs.rename('/Applications/4Casters/server2/db/store_temp.json', '/Applications/4Casters/server2/db/store.json', function(err) {
              if(err){
...
if(!fs.existsSync("/Applications/4Casters/server2/tempFile")){
  fs.mkdir("/Applications/4Casters/server2/tempFile/", 0766, function(err){
    if(err){
      console.log("error1");
      console.log("ERROR! Can't make the directory: "+err);
    }
  ...
  var updateLocalServer = function(algoId,newServerObj,newAlgoVersion,serverName,callback){
    ...
    db.update({ _id: algoId }, { $set: action }, function () {
      ...
      db.update({ _id: algoId }, { $set: {algo_version: newAlgoVersion } }, function () {
        ...
        var path ="/Applications/4Casters/server2/"+algoId;
        console.log("delete, url: ",path);
        var result = deleteFolderRecursive(path);
        var path2 ="/Applications/4Casters/server2/tempFile/"+algoId;
...
app.get('/updateSettingNewAlgo', function(req, res) {
  ...
  console.log("algo version: ",localAlgoVersion);
  var algoDetails = req.headers['algo-details'];
  var algoId = req.query.tmpAlgoId;

  var updateSetting = function(algoId,newServerObj,newAlgoVersion,serverName,callback){
    console.log("algo version: ",newAlgoVersion);
    var action = {};
    action[serverName] = newServerObj;
    db.update({ _id: algoId }, { $set: action }, function () {
      db.persistence.compactDatafile();
  ...
  db.find({ _id: algoId }, function (err, docs) {
    ...
    if (docs.length > 0) {
      ...
      if (docs[0].algo_version > localAlgoVersion) {
        ...
        console.log("in 2 qupdateSettingNewAlgo: ",docs[0][serverName]);
...
app.post('/uploadOnBeta', function(req, res) {
  ...
  var deleteFolderRecursive = function(path) {
	...
	if(!fs.existsSync("/Applications/4Casters/server2")){
  ...
  db.find({ _id: algoId }, function (err, docs) {
    ...
    if (docs.length > 0) {
      ...
      if (docs[0].algo_version > localAlgoVersion) {
        ...
        if(fs.existsSync("/Applications/4Casters/server2/"+algoId)){
          ...
          if(fs.existsSync("/Applications/4Casters/server2/tempFile"+algoId)){
            deleteFolderRecursive('/Applications/4Casters/server2/tempFile/'+algoId);
          }
          fs.mkdir("/Applications/4Casters/server2/tempFile/"+algoId, 0766, function(err){
            if(err){
              console.log("error1");
              console.log("ERROR! Can't make the directory: "+err);
            }else{
              var target = '/Applications/4Casters/server2/tempFile/'+algoId+'/'+nameFile;
              var wr = fs.createWriteStream(target);
              wr.on("error", function(err) {
              ...
              wr.on("close", function(ex) {
                ...
                fs.mkdir("/Applications/4Casters/server2/"+algoId, 0766, function(err){
                  if(err){
                    console.log("error1");
                    console.log("ERROR! Can't make the directory: "+err);
                  }else{
                    console.log("err 0: ");
                    mv('/Applications/4Casters/server2/tempFile/'+algoId+'/'+nameFile, '/Applications/4Casters/server2/'+algoId+'/'+nameFile, function(err) {
                      ...
                      if(err == undefined || err == null || err == ""){
                        ...
                        updateSetting(docs[0]['_id'],docs[0]['betaTest'],docs[0].algo_version,'betaTest',function(result){
                          ...
                          if (result == true) {
                            ...
                            deleteFolderRecursive('/Applications/4Casters/server2/tempFile/'+algoId);
                            deleteFolderRecursive('/Applications/4Casters/server2/'+algoId);
                            res.status(200).send({error:1,msg:"Error to create Algo",algoId:algoId}); 
                          }
                        }); 
                  ...
                  mv('/Applications/4Casters/server2/tempFile/'+algoId+'/'+nameFile, '/Applications/4Casters/server2/'+algoId+'/'+nameFile, function(err) {
                    ...
                    if(err == undefined || err == null || err == ""){
                      ...
                      db.insert(tempJson2, function (err, newDoc) {   
                        ...
                        updateSetting(newDoc['_id'],newDoc['betaTest'],newDoc.algo_version,'betaTest',function(){
                  ...
                  mv('/Applications/4Casters/server2/tempFile/'+algoId+'/'+nameFile, '/Applications/4Casters/server2/'+algoId+'/'+nameFile, function(err) {
                    ...
                    if(err == undefined || err == null || err == ""){
                      ...
                      db.insert(tempJson2, function (err, newDoc) {   
                        ...
                        newDoc.algo_version = newDoc.algo_version + 1;
                        updateSetting(newDoc['_id'],newDoc['prod'],newDoc.algo_version,'prod',function(result){
...
app.get('/updateSettingSkipTest', function(req, res) {
  ...
  var updateSetting = function(algoId,newServerObj,newAlgoVersion,serverName,callback){
    ...
    db.update({ _id: algoId }, { $set: action }, function () {
      db.persistence.compactDatafile();
      console.log("updated server obj to to skip test");
