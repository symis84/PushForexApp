    ...
    $scope.moveFile = function(sourcePath,destPath){
      ...
      $('#progress_bar').find('.percent').text( '0%');
        ...
        console.log("file.name: "+file.name);
      	if(!fs.existsSync("/Applications/4Casters/")){
          	fs.mkdir("/Applications/4Casters/", 0766, function(err){
             		if(err){console.log("ERROR! Can't make the directory: "+err)}
          	});  
      	}
        if(!fs.existsSync("/Applications/4Casters/tempFile")){
          fs.mkdir("/Applications/4Casters/tempFile", 0766, function(err){
            if(err){console.log("ERROR! Can't make the directory: "+err)}
          });  
        }
        var result = $scope.deletAllFilesInFolder('/Applications/4Casters/tempFile');
        if (result) {
          var fileDestPath = '/Applications/4Casters/tempFile/'+file.name;
        ...
        if(err != undefined && err != null && err != ""){
          ...
          storedb('algos').insert($scope.newAlgo,function(err,result){
            if(err == undefined || err == null || err == ""){
              ...
              if(!fs.existsSync("/Applications/4Casters/"+hash)){
                  fs.mkdir("/Applications/4Casters/"+hash, 0766, function(err){
                      if(err){
                        console.log("ERROR! Can't make the directory: "+err);
                        //TODO GESTISCI ERROR SE MKDIR FALLISCE
                      }else{

                        fs.readdir('/Applications/4Casters/tempFile/', function(err, items) {    
                          if(err == undefined || err == null || err == ""){               
                            console.log(items[0]);
                            //$scope.moveFile
                            mv('/Applications/4Casters/tempFile/'+items[0], '/Applications/4Casters/'+hash+"/"+items[0], function(err) {
                              console.log("err: ",err);
                              if(err == undefined || err == null || err == ""){
                                $scope.deletAllFilesInFolder('/Applications/4Casters/tempFile');
              ...
              if (body.error == 0 || body.error == 2) {
                ...
                updateDb(search,function(result){
                  ...
                  if (result == true) {
                    ...
                    var path ="/Applications/4Casters/"+tmpAlgoId;
          ...
          if(err == undefined || err == null || err == ""){ 
            ...
            $scope.updateDb(search,query,function(result){
              if (result == true) {
                ...
                var rd_file = fs.createReadStream('/Applications/4Casters/'+algoId+'/'+algoPath);
